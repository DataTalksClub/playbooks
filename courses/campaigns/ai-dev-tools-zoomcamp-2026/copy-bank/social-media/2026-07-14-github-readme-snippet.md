...
status: published
calendar_angle: Add 2026 cohort dates registration links and workshop schedule
---
...
You will compare modern AI developer tools, build and deploy a full-stack application, extend coding agents with MCP and reusable capabilities, and add observability, incident response, audit, and DevOps workflows around AI-built apps.
...
Add observability, alerting, incident response, audit, and bounded agent investigation around a deployed AI-built app.

You will:

- Instrument one important endpoint end to end with OpenTelemetry.
- Run metrics, logs, and traces behind a Collector with Prometheus, Loki, Tempo, and Grafana.
- Write alerts with useful context: symptom, dashboard link, deployed version, and runbook.
- Collect bounded evidence packets with allowlisted read-only queries.
- Use a coding agent as a read-only investigator that returns facts, ranked hypotheses, a proposed action, and uncertainty.
- Keep authorization outside the model with action allowlists and autonomy levels.
- Audit both the code and the responder's capabilities.

Example tools include OpenTelemetry, Prometheus, Loki, Tempo, Grafana, PR-Agent, Semgrep MCP, Snyk Agent Scan, K8sGPT, HolmesGPT, LiteLLM, and Ollama.

Outcome: a production feedback loop for your final project, with observability, alerting, read-only agent investigation, escalation policy, recovery verification, and audit.
